...
import pygame as pg
import time
from cards_and_piles import Card, Pile, StockPile, StockPileOpened, GrowingPile, SuitPile
from constants import VALUES, SUITS, CARD_SCALE, CARD_SPACING, NUM_GROWING_PILES, FPS, RANDOM_POP, \
    PILE_SPACING, EXTRA_PILE_SPACING, BOARD_SIZE, AUTO_WIN, PIXEL_SCALE
from actions import ActionManager, CardAction
import asyncio

pg.init()
pg.display.set_caption('Solitaire')
pg.display.set_icon(pg.image.load(os.path.join("assets", "card_back.png")))
win = pg.display.set_mode((800, 500))


async def main():
    solitaire = InSolitaire(win)
    run = True
    prev_time = time.time()
    while run:
...
class InSolitaire:
    def __init__(self, win):
        ...
        self.resetButton.options.append(
            Button(img=self.images['easy'], pos=(self.resetButton.rect.x, self.resetButton.rect.bottom + PILE_SPACING)))
        self.resetButton.options.append(Button(img=self.images['hard'], pos=(
        self.resetButton.options[-1].rect.x, self.resetButton.options[-1].rect.bottom + PILE_SPACING)))
    ...
    def reset(self, num_opened_cards):
        ...
        random.shuffle(self.stockPile.array)
            ...
            if pile.clickRect.collidepoint(pos):
                ...
                if cards is not None:
                    ...
                    if self.sourcePile in self.growingPiles:
                        self.dragging_pos = (self.sourcePile.rect.x - pos[0],
                                             self.sourcePile.rect.y + len(self.sourcePile.array) * CARD_SPACING - pos[
                                                 1])
                    else:
        ...
        for pile in chain(self.growingPiles, self.suitPiles):
            if pile.acceptRect.collidepoint(pos) and pile != self.sourcePile:
                if pile.check_accepted(pos, self.inHand) == 'accepted':
                    ...
                    self.actionManager.add_action(
                        CardAction(self.sourcePile, pile, num_cards, new_card_is_opened=new_card_is_opened))
                    pile.set_click_rect()
    ...
    def set_positions(self):
        ...
        for i, pile in enumerate(self.growingPiles):
            pile.acceptRect.topleft = pile.rect.topleft = (
            self.stockPile.rect.right + EXTRA_PILE_SPACING + (CARD_SCALE[0] + PILE_SPACING) * i,
            self.board_pos.y)
        ...
        for i, pile in enumerate(self.suitPiles):
            ...
            pile.acceptRect.update(pile.acceptRect.x, 0, self.win.get_width() - pile.acceptRect.x,
                                   self.win.get_height())
    ...
    def draw(self, win):
        ...
        if self.mousedown:
            if len(self.inHand.array) > 0:
                self.inHand.rect.topleft = (
                pg.mouse.get_pos()[0] + self.dragging_pos[0], pg.mouse.get_pos()[1] + self.dragging_pos[1])
